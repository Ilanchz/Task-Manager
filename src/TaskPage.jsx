...
function TaskPage(props){
        ...
        async function addNewTask(event){
                ...
                if (tname!=="" && sdate!=="" && deadline!==""){
                        if (name !== "") {
                                ...
                                const response = await fetch(`http://localhost:5000/api/pushNewTask/${name}`, {
                                  method: 'POST',
                                  headers: {
                                    'Content-Type': 'application/json',
                                  },
                                  body: JSON.stringify([{ "name": tname, "startdate": sdate, "deadline": deadline }]),
                                });
                              
                                // Handle the response if needed
                        }
                        let tasks_arr = [...tasks, { "name": tname, "startdate": sdate, "deadline": deadline }];
                        setTask(tasks_arr);
        ...
        <button id="task-button" className="hover:border-l-sky-950 rounded-xl w-16 h-16" onClick={addNewTaskDiv}><svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 128 128">
